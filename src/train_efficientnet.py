"""
Train EfficientNet Baseline Model for Brain Tumor Classification
"""
...
from preprocessing import prepare_brisc_data, prepare_nickparvar_data, verify_batch

print("="*70)
print("🧠 TRAINING EFFICIENTNET BASELINE MODEL")
print("="*70)
...
for epoch in range(NUM_EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), os.path.join(MODELS_DIR, "efficientnet_best.pth"))
...
ax1.set_ylabel('Loss')
ax1.set_title('EfficientNet - Training & Validation Loss')
ax1.legend()
...
ax2.set_title('EfficientNet - Training & Validation Accuracy')
ax2.legend()
ax2.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(os.path.join(RESULTS_DIR, "efficientnet_training.png"), dpi=300)
plt.show()
print(f"✅ Training plot saved to: {RESULTS_DIR}/efficientnet_training.png")
...
# Load best model
model.load_state_dict(torch.load(os.path.join(MODELS_DIR, "efficientnet_best.pth")))
...
def plot_confusion_matrix(cm, dataset_name, save_name):
    ...
    plt.title(f'EfficientNet - Confusion Matrix ({dataset_name})', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, save_name), dpi=300)
    plt.show()
    print(f"✅ Confusion matrix saved to: {RESULTS_DIR}/{save_name}")

plot_confusion_matrix(brisc_results['confusion_matrix'], "BRISC Test", "efficientnet_brisc_cm.png")
plot_confusion_matrix(nick_results['confusion_matrix'], "Nickparvar Test", "efficientnet_nickparvar_cm.png")
...
# Save to CSV
results_df.to_csv(os.path.join(RESULTS_DIR, "efficientnet_results.csv"), index=False)
print(f"\n✅ Results saved to: {RESULTS_DIR}/efficientnet_results.csv")
...
ax.set_title('EfficientNet - ROC Curves', fontsize=14, fontweight='bold')
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(RESULTS_DIR, "efficientnet_roc.png"), dpi=300)
plt.show()
print(f"✅ ROC curves saved to: {RESULTS_DIR}/efficientnet_roc.png")
...
Model saved to:   {MODELS_DIR}/efficientnet_best.pth
""")